    ...
    | Some "Enter" ->
      let num =
        ...
        |> Option.bind ~f:(fun el ->
               Option.try_with (fun () -> el##.value |> Js.to_string |> Int.of_string))
  ...
  let make_one ~key:label ~data:action acc =
    ...
    :: Node.label
         Attr.
           [
             for_ id_;
             classes [ "btn"; "btn-outline-primary"; (if is_selected then "active" else "inactive") ];
           ]
         [ Node.text label ]
    :: acc
  in
...
let render_show_all_checkbox ~update_show_all show_all =
  let id_ = "show-all-checkbox" in
  let attrs =
    Attr.
      [
        ...
        on_input (fun _ev _s ->
            let checked =
              let open Js_of_ocaml in
              Dom_html.getElementById_coerce id_ Dom_html.CoerceTo.input
              |> Option.value_map ~default:false ~f:(fun x -> x##.checked |> Js.to_bool)
            in
            update_show_all checked);
      ]
    |> add_if show_all Attr.checked
  in
  Node.div
    Attr.[ classes [ "form-check"; "form-switch" ] ]
    [
      Node.input attrs [];
      Node.label Attr.[ class_ "form-check-label"; for_ id_ ] [ Node.text "Show All" ];
    ]
...
let render_items ~update_backpack ~update_selected backpack selected =
  ...
  let render_category items =
    String.Map.fold_right items ~init:[] ~f:(fun ~key:_ ~data:(item, num) acc ->
        let handler = Attr.on_click (fun _ev -> update_selected (Some item)) in
        let node =
          let first_col =
            Node.div []
              [
                  ...
                  (Attrs.merge_classes_and_styles
                     Attr.
                       [
                         pointer;
                         unselectable;
                         src (Glossary.to_img_uri current_uri item |> Uri.to_string);
                         handler;
                       ])
                  [];
              ]
          ...
          let second_col =
            let quantity =
              match [%equal: Glossary.t option] (Some item) selected with
          ...
          Node.div
            Attr.
              [
                style Css_gen.(width (`Em 14) @> height (`Em 11));
...
let component ~inventory () =
  ...
  @@ let%map backpack, update_backpack = backpack
     ...
     let show_all_node = render_show_all_checkbox ~update_show_all show_all in
     ...
     let items_node =
       ...
       render_items ~update_backpack ~update_selected organized selected
     in
     (backpack, update_backpack), `Organize buttons_node, `Show_all show_all_node, `Items items_node
